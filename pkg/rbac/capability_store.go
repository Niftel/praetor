...
// CapabilityStore owns Praetor's role-definition and assignment persistence.
// Its read helpers remain useful for management endpoints and migration tests;
// runtime authorization decisions are evaluated by pkg/authorization and RBAC v4.
//
// It uses the capability schema in this repository:
...
//
// It is agnostic only to WHICH resource types are protected: the "every id of
...
// or to team_members. The matching migrations live under db/migrations.
//
// The is_superuser bypass is NOT here; it lives in WithSystemFlags,
...
// object and is handled by VisibleIDs, so it is not expanded here.
