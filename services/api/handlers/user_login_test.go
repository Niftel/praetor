...
func TestUserCreateLogin(t *testing.T) {
	...
	rec := callJSON(t, h.CreateUser, http.MethodPost, fmt.Sprintf(`{"username":%q}`, uname), super, nil)
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("create without password: want 400, got %d", rec.Code)
	}

	// Create with a real password.
	rec = callJSON(t, h.CreateUser, http.MethodPost,
	...
	rec = callJSON(t, h.Login, http.MethodPost, fmt.Sprintf(`{"username":%q,"password":"s3cret-pw"}`, uname), anon, nil)
	...
	rec = callJSON(t, h.Login, http.MethodPost, fmt.Sprintf(`{"username":%q,"password":"nope"}`, uname), anon, nil)
	...
	rec = callJSON(t, h.UpdateUser, http.MethodPut,
	...
	rec = callJSON(t, h.Login, http.MethodPost, fmt.Sprintf(`{"username":%q,"password":"reset-pw"}`, uname), anon, nil)
	if rec.Code != http.StatusOK {
